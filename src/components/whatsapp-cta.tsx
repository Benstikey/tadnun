...
export function WhatsAppCta() {
  ...
  const sectorKey = getSectorFromParams(
    Object.fromEntries(searchParams.entries())
  );
  const sectorName = sectorKey
    ? tSectors(`items.${sectorKey}.name`)
    : null;

  const prefillText = sectorName
    ? t("whatsappPrefillSector", { sector: sectorName })
    : t("whatsappPrefill");
  ...
  return (
    ...
    >
      ...
      <svg
        ...
        viewBox="0 0 16 16"
        fill="none"
        className="opacity-70 group-hover:translate-x-0.5 transition-transform rtl:-scale-x-100"
      >
        <path
