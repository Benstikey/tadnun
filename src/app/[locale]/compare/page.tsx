            ...
            {pages.map((page, i) => (
              <ScrollReveal key={page.slug} delay={i * 60}>
                <a
                  ...
                  className="group flex flex-col gap-3 rounded-2xl border border-border bg-surface p-6 transition-all duration-300 hover:border-foreground/15 hover:shadow-md hover:-translate-y-0.5 h-full"
                >
                  <span className="text-[10px] font-mono uppercase tracking-wider text-accent bg-accent/[0.06] rounded-full px-2.5 py-0.5 self-start">
                    {page.type === "vs" ? "VS" : page.type === "alternative" ? tCompare("altLabel") : tCompare("categoryLabel")}
                  </span>
                  <h2 className="font-serif italic text-foreground text-[17px] leading-snug tracking-tight group-hover:text-accent transition-colors">
                    {page.title}
                  </h2>
